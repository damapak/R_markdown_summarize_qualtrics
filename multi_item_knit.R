...
if(winDialog(type="yesno", "Do you want to summarize a subset of your data?") == "YES"){
  ...
  fieldselect <- select.list(fieldlist, graphics= TRUE, multi = FALSE, title="Select Field to Filter by")
  valuelist = names(table(surveydata[,fieldselect]))
  valueselect = select.list(valuelist, graphics= TRUE, multi = TRUE, title="Select Desired Values")
